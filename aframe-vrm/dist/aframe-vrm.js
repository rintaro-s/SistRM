(function (global, factory) {
    ...
    AFRAME.registerSystem('vrm-network', {
        init() {
        ...
        handleMessage(msg) {
            switch (msg.type) {
                case 'full_state':
                    for (const user of msg.users || msg.entities || []) {
                        this.updateRemoteAvatar(user.user_id, user);
                    }
                    break;
                case 'avatar_delta':
                    this.updateRemoteAvatar(msg.user_id, msg);
                    break;
                case 'user_joined':
                    this.spawnRemoteAvatar(msg.user_id, msg.avatar_url);
                    break;
                case 'user_left':
                    this.removeRemoteAvatar(msg.user_id);
                    break;
            }
        },
        spawnRemoteAvatar(userId, avatarUrl) {
            if (this.remoteAvatars.has(userId))
                return;
            if (!avatarUrl)
                return;
            ...
            entity.id = `remote-avatar-${userId}`;
            scene.appendChild(entity);
            const loader = new GLTFLoader();
            loader.register((parser) => new VRMLoaderPlugin(parser));
            const remote = { entity, loader };
            this.remoteAvatars.set(userId, remote);
            // Hook into model load
            entity.addEventListener('model-loaded', (evt) => {
                const vrm = evt.detail.model;
                if (vrm) {
                    remote.vrm = vrm;
                    const vrmSystem = scene.systems['vrm'];
                    if (vrmSystem && vrmSystem.registerVRM) {
                        vrmSystem.registerVRM(vrm);
                    }
                }
            });
        },
        updateRemoteAvatar(userId, data) {
            let remote = this.remoteAvatars.get(userId);
            if (!remote) {
                // Auto-spawn if we have avatar_url
                if (data.avatar_url) {
                    this.spawnRemoteAvatar(userId, data.avatar_url);
                    remote = this.remoteAvatars.get(userId);
                }
                if (!remote)
                    return;
            }
            ...
            if (transform) {
                ...
                // Simple lerp interpolation could be added here
                remote.entity.setAttribute('position', { x: pos[0], y: pos[1], z: pos[2] });
                remote.entity.object3D.quaternion.set(rot[0], rot[1], rot[2], rot[3]);
                remote.entity.object3D.scale.set(scale[0], scale[1], scale[2]);
            }
            if (remote.vrm) {
                const expressions = data.expressions;
                if (expressions && remote.vrm.expressionManager) {
                    for (const [name, value] of Object.entries(expressions)) {
                        remote.vrm.expressionManager.setValue(name, value);
                    }
                }
                const lookAt = data.look_at;
                if (lookAt && remote.vrm.lookAt) {
                    remote.vrm.lookAt.target.set(lookAt[0], lookAt[1], lookAt[2]);
                }
            }
        },
        removeRemoteAvatar(userId) {
            const remote = this.remoteAvatars.get(userId);
            if (!remote)
                return;
            if (remote.vrm) {
                const vrmSystem = this.sceneEl.systems['vrm'];
                if (vrmSystem && vrmSystem.unregisterVRM) {
                    vrmSystem.unregisterVRM(remote.vrm);
                }
                VRMUtils.deepDispose(remote.vrm.scene);
            }
    ...
    AFRAME.registerComponent('vrm-expressions', {
        ...
        updateExpressions() {
            const modelComponent = this.el.components['vrm-model'];
            if (!modelComponent || !modelComponent.vrm)
                return;
            const vrm = modelComponent.vrm;
            if (!vrm.expressionManager)
                return;
            for (const key of Object.keys(this.data)) {
                const value = parseFloat(this.data[key]);
                if (!isNaN(value)) {
                    vrm.expressionManager.setValue(key, Math.max(0, Math.min(1, value)));
                }
            }
        },
    });
    ...
    AFRAME.registerComponent('vrm-look-at', {
        ...
        updateTarget() {
            ...
            if (this.data.type === 'bone') {
                vrm.lookAt.applier = vrm.lookAt.applier;
                // Type is set during construction; we just update target
            }
        },
        tick() {
            const modelComponent = this.el.components['vrm-model'];
            ...
            if (this.targetEntity && this.targetEntity.object3D) {
                const worldPos = new THREE42__namespace.Vector3();
                this.targetEntity.object3D.getWorldPosition(worldPos);
                vrm.lookAt.target.copy(worldPos);
            }
            else {
    ...
    AFRAME.registerComponent('vrm-spring-bone', {
        ...
        updateSpringBone() {
            ...
            if (!vrm.springBoneManager)
                return;
            // three-vrm spring bone manager applies settings per-joint from VRM data.
            // Runtime override of global gravity is not directly exposed,
            // but we document the schema for future implementation.
            console.log('[vrm-spring-bone] Settings applied:', this.data);
        },
    });
